...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    getDashboard()
      ...
      .catch((e) => {
        if (!ignore) {
          setError(
            e.response?.data?.error ?? "Failed to load dashboard data",
          );
        }
      });
  ...
  if (error) {
    return (
      <div className="px-5 pb-8">
        ...
        <FinancialAdvisorModal open={advisorOpen} onClose={() => setAdvisorOpen(false)} />
      </div>
    );
  }
  ...
  return (
    <div className="px-5 pb-8 space-y-6">
      <Header name={dashboard.greetingName} onOpenAI={() => setAdvisorOpen(true)} />

      {/* Stale-data badge — shown only when data comes from the offline cache */}
        ...
        <section>
          ...
          {dashboard.overview.length > 0 ? (
            ...
            <Card className="p-5 text-center space-y-2">
              ...
              <Link href="/profile" className="inline-block text-xs font-bold text-primary hover:underline">
                Setup Income →
              </Link>
            </Card>
          )}
...
function StaleBadge({ cachedAt }: { cachedAt: number }) {
  return (
    <div className="flex items-center gap-2 rounded-xl bg-amber-500/10 border border-amber-500/20 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
      <WifiOff size={13} />
      <span>
        Showing offline data — last updated {formatRelativeTime(cachedAt)}
      </span>
    </div>
  );
}
